...
export async function generateShotVideo(req: ShotVideoRequest): Promise<ShotVideoResponse> {
  const resp = await fetch(`${API_BASE}/shots/video`, {
    ...
    body: JSON.stringify({
      ...
      mode: req.mode,
      duration_seconds: req.duration_seconds ?? 15.0,
      seed: req.seed,
...
export async function extractVideoKeyframes(
  ...
  const resp = await fetch(`${API_BASE}/pipeline/extract-keyframes`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ project_id: projectId, video_url: videoUrl, count, sample_fps: 1.0 }),
  });
  if (!resp.ok) throw new Error(`Failed to extract keyframes: ${await resp.text()}`);
  return resp.json();
}

export async function startVideoUpscale(
  projectId: string,
  videoUrl: string,
  scale: number = 2,
): Promise<UpscaleStatus> {
  const resp = await fetch(`${API_BASE}/pipeline/upscale`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ project_id: projectId, video_url: videoUrl, scale }),
  });
  if (!resp.ok) throw new Error(`Failed to start upscale: ${await resp.text()}`);
  return resp.json();
}

export async function getVideoUpscaleStatus(jobId: string): Promise<UpscaleStatus> {
  const resp = await fetch(`${API_BASE}/pipeline/upscale/status/${encodeURIComponent(jobId)}`);
  if (!resp.ok) throw new Error(`Failed to check upscale: ${await resp.text()}`);
  return resp.json();
}
