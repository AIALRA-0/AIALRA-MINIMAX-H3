...
import { Film, Image as ImageIcon, Loader2, PanelLeftClose, PanelLeftOpen, Sparkles, Video, ChevronDown, ChevronUp, Settings, Upload, Move3d } from "lucide-react";
      ...
      <header className="h-14 bg-studio-panel/80 backdrop-blur-md border-b border-studio-border flex items-center px-4 shrink-0">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 text-xs">
            ...
            <span className="text-studio-muted hidden sm:inline">{connected ? t("status.connected") : t("status.offline")}</span>
          </div>
          ...
          >
            ...
            <span className="hidden sm:inline">{t("nav.previs")}</span>
          ...
          <aside className={`bg-studio-panel/50 border-r border-studio-border overflow-hidden shrink-0 transition-all duration-200 ${assetPanelCollapsed ? "w-10" : "w-56"}`}>
            {assetPanelCollapsed ? (
              <div className="flex flex-col items-center pt-3 gap-3">
                ...
                <ImageIcon className="w-4 h-4 text-studio-muted/50" />
              </div>
            ) : (
            ...
            {inspectorCollapsed ? (
              ...
              <div className="h-full flex flex-col">
                <div className="flex items-center justify-between px-2 pt-2 pb-1 bg-studio-panel/50 border-b border-studio-border shrink-0">
                  <span className="text-[10px] text-studio-muted/50 uppercase tracking-wider">{t("nav.inspector")}</span>
                  <button
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 bg-studio-panel/50 border-b border-studio-border shrink-0 overflow-x-auto">
                  ...
                  <div className="flex items-center gap-1 overflow-x-auto">
                    {[...shots].sort((a, b) => (a.sequence_order ?? 0) - (b.sequence_order ?? 0)).map((shot, idx) => (
                      ...
                      >
                        <div className="relative w-16 h-9 bg-studio-bg flex items-center justify-center">
                          {shot.frame_image_path ? (
                            <NextImage src={shot.frame_image_path} alt={shot.name} fill sizes="64px" unoptimized className="object-cover" />
                          ) : (
                            <span className="text-[8px] text-studio-muted/40">{t("workspace.noFrame")}</span>
                          )}
                        </div>
                    ...
                    {shots.length === 0 && (
                      <span className="text-[10px] text-studio-muted/40">{t("workspace.noShots")}</span>
                    )}
                  </div>
                  <button
