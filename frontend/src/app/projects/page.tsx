      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-studio-muted">{t("projects.subtitle")}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <LanguageToggle />
        ...
        {showCreate && (
          <div className="mb-6 p-5 bg-studio-panel border border-studio-border rounded-xl">
            <h2 className="text-sm font-semibold mb-4">{t("projects.createTitle")}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-[11px] font-medium text-studio-muted mb-1 block">{t("projects.name")}</label>
                <input
                  value={newName}
              ...
              <div>
                <label className="text-[11px] font-medium text-studio-muted mb-1 block">{t("projects.description")}</label>
                <input
                  value={newDesc}
        ...
        {loading ? (
          <div className="text-center py-16 text-studio-muted text-sm">{t("projects.loading")}</div>
        ) : projects.length === 0 ? (
          <div className="text-center py-16">
            <FolderOpen className="w-12 h-12 text-studio-muted/30 mx-auto mb-4" />
            <p className="text-studio-muted text-sm mb-2">{t("projects.empty")}</p>
            <p className="text-studio-muted/50 text-xs">{t("projects.emptyHint")}</p>
          </div>
        ) : (
              ...
              >
                <Link href={`/project/${project.id}`} className="block">
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-studio-muted/50">
                    ...
                    {new Date(project.created_at).toLocaleDateString(locale === "zh-CN" ? "zh-CN" : "en-US")}
                  </div>
                </Link>
