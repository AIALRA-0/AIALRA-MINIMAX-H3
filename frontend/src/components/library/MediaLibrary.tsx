...
const typeFilters = [
  ...
  { id: "character", label: "Characters", labelZh: "角色" },
  { id: "location", label: "Locations", labelZh: "场景" },
  { id: "prop", label: "Props", labelZh: "道具" },
  { id: "vehicle", label: "Vehicles", labelZh: "载具" },
  { id: "style", label: "Styles", labelZh: "风格" },
  { id: "effect", label: "Effects", labelZh: "特效" },
];
...
export function AssetLibrary({ projectId, mode = "default" }: AssetLibraryProps) {
  ...
  const handleDelete = async (assetId: string) => {
    if (!confirm(zh ? "确定删除这个素材吗" : "Delete this asset?")) return;
    try {
  ...
  const handleDeleteVideoAsset = async (video: VideoAsset) => {
    if (!window.confirm(zh ? `确定删除视频“${video.filename}”吗` : `Delete video '${video.filename}'?`)) return;
    try {
  ...
  const handleDeleteImageAsset = async (image: ImageAssetItem) => {
    if (!window.confirm(zh ? `确定删除图片“${image.filename}”吗` : `Delete image '${image.filename}'?`)) return;
    try {
  ...
  const handleDeleteAudioFile = async (item: AudioFileItem) => {
    if (!window.confirm(zh ? `确定删除音频“${item.filename}”吗` : `Delete audio '${item.filename}'?`)) return;
    try {
  ...
  return (
    ...
    <div className="p-3 overflow-y-auto h-full">
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-xs font-semibold text-studio-muted uppercase tracking-wider">{zh ? "素材库" : "Library"}</h2>
        <button
          ...
          title={zh ? "上传文件并自动识别类型" : "Upload file (auto-categorizes by type)"}
        >
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder={zh ? "搜索素材、镜头和音频…" : "Search assets, clips, audio..."}
      ...
      <div className="mb-4">
        ...
        {expandedSections.assets && (
          <>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {filtered.map((asset) => {
                ...
                return (
                  ...
                  >
                    {thumbUrl ? (
                      <Image unoptimized src={thumbUrl} alt={asset.name} width={256} height={256} className="w-full h-full object-cover" />
                    ) : (
            ...
            {filtered.length === 0 && (
              <p className="text-[11px] text-studio-muted/50 px-2 py-2">{zh ? "还没有素材" : "No assets yet"}</p>
            )}
          </>
        )}
      </div>
      ...
      {selectedAsset && (
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              ...
              {zh ? "视频镜头" : "Video Clips"} ({shotsWithVideo.length + filteredVideoAssets.length})
            </button>
            <button
              ...
              title={zh ? "清理已丢失的视频引用" : "Clean up missing video files"}
            >
          ...
          {expandedSections.videos && (
            <div className="space-y-2">
              {storeShots.length === 0 && <p className="text-[11px] text-studio-muted">{zh ? "加载中…" : "Loading..."}</p>}

              {/* Uploaded videos pseudo-scene group (top) */}
              {filteredVideoAssets.length > 0 && (
                <div>
                  <p className="text-[10px] text-studio-muted uppercase tracking-wider px-2 mb-1">{zh ? "已上传" : "Uploaded"} ({filteredVideoAssets.length})</p>
                            ...
                            >
                              <div className="w-8 h-8 rounded overflow-hidden bg-studio-bg shrink-0 relative">
                                {shot.frame_image_path && (
                                  <Image unoptimized src={shot.frame_image_path} alt="" width={96} height={96} className="w-full h-full object-cover" />
                                )}
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-[10px] text-studio-muted">
                                  {hasMultipleTakes ? `${takes.length} ${zh ? "个候选" : "takes"}` : (zh ? "视频" : "Video")}
                                </p>
                              </div>
                              {hasMultipleTakes && (
                                <button
                              ...
                              <div className="ml-4 mt-1 space-y-1 border-l border-studio-border pl-2">
                                {takes.map((take: any) => (
                                  ...
                                  >
                                    ...
                                    <button
                                      onClick={(e) => { e.stopPropagation(); if (confirm(zh ? `确定删除候选镜头 ${take.id} 吗` : `Delete take ${take.id}?`)) handleDeleteTakeInLibrary(shot.id, take.id); }}
                                      className="p-0.5 rounded hover:bg-red-500/20 text-studio-muted hover:text-red-400 opacity-100 transition-all shrink-0"
                                      title="Delete take"
                                    >
                                      <Trash2 className="w-3 h-3" />
              ...
              {shotsWithVideo.length === 0 && filteredVideoAssets.length === 0 && (
                <p className="text-[11px] text-studio-muted/50 px-2">{zh ? "还没有视频镜头" : "No video clips yet"}</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="mb-4">
          ...
          >
            ...
            {zh ? "分镜帧" : "Storyboard Frames"} ({shotsWithFramesOnly.length})
          </button>
          {expandedSections.frames && (
            <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      {filtered.map((shot) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-studio-text truncate">{shot.name}</p>
                            <p className="text-[10px] text-studio-muted">{isCameraMode ? (zh ? "画面" : "Frame") : (zh ? "静帧，保持 5 秒" : "Frame (5s hold)")}</p>
                          </div>
                          {isCameraMode ? (
                            <Camera className="w-3.5 h-3.5 text-studio-muted group-hover:text-studio-accent shrink-0" />
                          ) : (
              ...
              {shotsWithFramesOnly.length === 0 && (
                <p className="text-[11px] text-studio-muted/50 px-2">{zh ? "还没有分镜帧" : "No storyboard frames"}</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              ...
              {zh ? "图片" : "Images"} ({filteredImageAssets.length})
            </button>
          </div>
          {expandedSections.images && (
            <div className="space-y-1">
              {imageAssetsLoading && <p className="text-[11px] text-studio-muted">{zh ? "加载中…" : "Loading..."}</p>}
              {filteredImageAssets.map((image) => (
                ...
                >
                  <div className="w-8 h-8 rounded overflow-hidden bg-studio-bg shrink-0">
                    <Image unoptimized src={image.image_url} alt="" width={96} height={96} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-studio-text truncate">{image.filename}</p>
                    <p className="text-[10px] text-studio-muted">{(image.size_bytes / 1024).toFixed(0)} KB · {zh ? "保持 5 秒" : "5s hold"}</p>
                  </div>
                  <button
              ...
              {filteredImageAssets.length === 0 && !imageAssetsLoading && (
                <p className="text-[11px] text-studio-muted/50 px-2">{zh ? "还没有上传图片" : "No uploaded images"}</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              ...
              {zh ? "音频" : "Audio"} ({audioFiles.length})
            </button>
          </div>
          {expandedSections.audio && (
            <div className="space-y-1">
              {audioLoading && <p className="text-[11px] text-studio-muted">{zh ? "加载中…" : "Loading..."}</p>}

              {filteredVoiceFiles.length > 0 && (
                <div className="mb-1">
                  <p className="text-[10px] text-studio-muted uppercase tracking-wider px-2 mb-0.5">{zh ? "人声" : "Voice"} ({filteredVoiceFiles.length})</p>
              ...
              {filteredMusicFiles.length > 0 && (
                <div className="mb-1">
                  <p className="text-[10px] text-studio-muted uppercase tracking-wider px-2 mb-0.5">{zh ? "音乐" : "Music"} ({filteredMusicFiles.length})</p>
              ...
              {filteredFoleyFiles.length > 0 && (
                <div className="mb-1">
                  <p className="text-[10px] text-studio-muted uppercase tracking-wider px-2 mb-0.5">{zh ? "拟音" : "Foley"} ({filteredFoleyFiles.length})</p>
              ...
              {filteredOtherAudioFiles.length > 0 && (
                <div className="mb-1">
                  <p className="text-[10px] text-studio-muted uppercase tracking-wider px-2 mb-0.5">{zh ? "其他" : "Other"} ({filteredOtherAudioFiles.length})</p>
              ...
              {audioFiles.length === 0 && !audioLoading && (
                <p className="text-[11px] text-studio-muted/50 px-2">{zh ? "还没有音频" : "No audio files"}</p>
              )}
            </div>
          )}
        </div>
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-studio-text">{zh ? "创建素材" : "Create Asset"}</p>
            </div>
            {/* Preview */}
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                ...
                title={zh ? "仅作为普通图片上传，不创建素材元数据" : "Upload as simple image (no asset metadata)"}
              >
                <Upload className="w-3.5 h-3.5" />
                {zh ? "快速上传" : "Quick Upload"}
              </button>
              <button
                onClick={handleCancelCreateAsset}
