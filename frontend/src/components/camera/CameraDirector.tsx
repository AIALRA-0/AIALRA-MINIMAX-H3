...
export function CameraDirector({ projectId }: { projectId: string }) {
  ...
  const [resolutionQuality, setResolutionQuality] = useState("draft");
    ...
    try {
      ...
      const resp = await generateLongTake({
        ...
        extra_params: {
          ...
          turbo_mode: resolutionQuality === "draft",
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-3xl mx-auto p-4">
        ...
        <div className="flex items-start gap-3 mb-4">
          <div className="relative w-12 h-12 rounded-lg overflow-hidden bg-studio-panel border border-studio-border shrink-0">
            {(selectedShot?.frame_image_path || selectedShot?.last_frame_path) ? (
              <NextImage src={selectedShot.frame_image_path || selectedShot.last_frame_path!} alt="" fill sizes="48px" unoptimized className="object-cover" />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-sm font-semibold text-studio-text truncate">
              {selectedShot ? selectedShot.name : t("studio.freestyle")}
            </h1>
            {selectedShot ? (
              <>
                ...
                <div className="flex items-center gap-2 mt-1">
                  {selectedShot.video_takes && selectedShot.video_takes.length > 0 ? (
                    ...
                    <span className="text-xs text-studio-muted/50">{t("studio.noTakes")}</span>
                  )}
                </div>
              </>
            ) : (
              <p className="text-sm text-studio-muted">{t("studio.noShot")}</p>
            )}
          </div>
        </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            <span className="px-2 py-0.5 text-[10px] rounded-full bg-studio-border/50 text-studio-muted">
              {t("studio.max", { seconds: caps.maxDuration })}
            </span>
        ...
        {mode !== "t2v" && !longTakeMode && (
          <div className="mb-3 space-y-1.5">
            <label className="text-[10px] font-semibold text-studio-muted uppercase tracking-wider mb-1.5 block">
              {mode === "i2v" ? t("studio.frames") : mode === "ia2v" ? t("studio.imageAudio") : t("studio.references")}
            </label>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-[10px] font-semibold text-studio-muted uppercase tracking-wider">
              {longTakeMode ? t("studio.globalPrompt") : t("studio.prompt")}
            </label>
          ...
          <textarea
            ...
            placeholder={longTakeMode ? t("studio.longPromptPlaceholder") : t("studio.promptPlaceholder")}
            rows={6}
        ...
        <div className="mb-3 grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2">
            <label className="text-[10px] font-semibold text-studio-muted uppercase tracking-wider mb-1.5 block">
               {t("studio.duration")} <span className="opacity-50">{t("studio.max", { seconds: caps.maxDuration })}</span>
            </label>
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {generating ? (
              ...
              <span className="flex items-center gap-1.5">
                 {longTakeMode ? t("studio.generateLongTake") : t("studio.generateTake")}
          ...
          <div className="mt-6 animate-fade-in">
            ...
            <div className="rounded-xl overflow-hidden border border-studio-border bg-studio-panel">
              ...
              <div className="px-3 pb-3">
                <PipelineActions projectId={projectId} videoUrl={freestyleResult.videoUrl} />
              </div>
            </div>
          </div>
        )}
...
function RefSlot({
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-studio-panel rounded-lg border border-studio-border">
      <div className="relative w-8 h-8 rounded-md bg-studio-bg flex items-center justify-center shrink-0 overflow-hidden">
        {type === "image" && value ? (
          <NextImage src={value} alt="" fill sizes="32px" unoptimized className="object-cover" />
        ) : (
                  ...
                  <div className="space-y-1.5">
                    {sameSceneVideos.map((shot) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-studio-panelHover transition-all text-left"
                      >
                        <div className="relative w-16 h-10 rounded bg-studio-bg overflow-hidden shrink-0">
                          {shot.frame_image_path && (
                            <NextImage src={shot.frame_image_path} alt="" fill sizes="64px" unoptimized className="object-cover" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
...
function PipelineActions({
  ...
  const { t } = useLocale();
  const [keyframes, setKeyframes] = useState<RankedKeyframe[]>([]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const result = await getVideoUpscaleStatus(upscaleJobId);
        if (cancelled) return;
        if (result.status === "completed" && result.delivery_video_url) {
          setMasterVideoUrl(result.delivery_video_url);
          setUpscaleStatus("pipeline.ready");
          setUpscaleJobId(null);
        } else if (result.status === "failed") {
          setPipelineError(result.error_message || t("pipeline.upscaleFailed"));
          setUpscaleStatus("");
          setUpscaleJobId(null);
        } else {
          setUpscaleStatus("pipeline.rendering");
          timer = window.setTimeout(poll, 5000);
        }
      } catch (error) {
        if (cancelled) return;
        setPipelineError(error instanceof Error ? error.message : t("pipeline.statusFailed"));
        setUpscaleStatus("");
        setUpscaleJobId(null);
      }
    };
  ...
  const handleExtract = async () => {
    ...
    try {
      const result = await extractVideoKeyframes(projectId, videoUrl, 8);
      setKeyframes(result.frames);
    } catch (error) {
      setPipelineError(error instanceof Error ? error.message : t("pipeline.extractFailed"));
    } finally {
      setExtracting(false);
    }
  };
  ...
  const handleUpscale = async () => {
    ...
    setUpscaleStatus("pipeline.submitting");
    try {
      const result = await startVideoUpscale(projectId, videoUrl, 2);
      if (result.status === "failed") throw new Error(result.error_message || t("pipeline.upscaleFailed"));
      setUpscaleJobId(result.job_id);
    } catch (error) {
      setPipelineError(error instanceof Error ? error.message : t("pipeline.upscaleFailed"));
      setUpscaleStatus("");
    }
  };

  return (
    <div className="space-y-2 border-t border-studio-border/60 pt-2">
      <div className="flex gap-1.5">
        <button
          onClick={handleExtract}
          disabled={extracting}
          className="flex-1 px-2 py-1.5 rounded-lg bg-studio-bg text-studio-text text-xs font-medium flex items-center justify-center gap-1 hover:bg-studio-border disabled:opacity-50 transition-colors"
        >
          {extracting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ImageIcon className="w-3.5 h-3.5" />}
          {extracting ? t("pipeline.extracting") : t("pipeline.extract")}
        </button>
        <button
          onClick={handleUpscale}
          disabled={Boolean(upscaleJobId)}
          className="flex-1 px-2 py-1.5 rounded-lg bg-studio-accent/20 text-studio-accent text-xs font-medium flex items-center justify-center gap-1 hover:bg-studio-accent/30 disabled:opacity-50 transition-colors"
        >
          {upscaleJobId ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wand2 className="w-3.5 h-3.5" />}
          {t("pipeline.upscale")}
        </button>
      </div>
      {upscaleStatus && <p className="text-[10px] text-studio-muted">{t(upscaleStatus)}</p>}
      {pipelineError && <p className="text-[10px] text-studio-danger">{pipelineError}</p>}
      {keyframes.length > 0 && (
        <div className={`grid ${compact ? "grid-cols-4" : "grid-cols-8"} gap-1`}>
          {keyframes.map((frame, index) => (
            <a key={frame.url} href={frame.url} target="_blank" rel="noreferrer" title={`${index + 1} · ${t("pipeline.score", { score: frame.score.toFixed(1) })}`}>
              <NextImage src={frame.url} alt={`${t("pipeline.extract")} ${index + 1}`} width={320} height={180} sizes={compact ? "100px" : "80px"} unoptimized className="aspect-video w-full rounded object-cover border border-studio-border" />
            </a>
          ))}
        </div>
      )}
      {masterVideoUrl && (
        <a href={masterVideoUrl} target="_blank" rel="noreferrer" className="block text-[11px] text-studio-accent hover:underline">
          {t("pipeline.openMaster")}
        </a>
      )}
    </div>
  );
    ...
    <>
      <div className={`relative rounded-xl overflow-hidden border-2 transition-all ${
        ...
        <div className="p-2.5 bg-studio-panel space-y-2">
          ...
          <PipelineActions projectId={projectId} videoUrl={take.path} compact />
        </div>
      </div>
