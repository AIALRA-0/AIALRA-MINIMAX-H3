...
export function ShotComposer({ projectId }: { projectId: string }) {
  ...
  const handleDuplicate = async (shot: ShotResponse) => {
    const dup = await createShot(projectId, `${shot.name} ${zh ? "副本" : "(copy)"}`, shot.description || "", shot.scene_id || undefined, shot.shot_type);
  ...
  const handleDelete = async (shotId: string) => {
    if (!confirm(zh ? "确定删除这个镜头吗" : "Delete this shot?")) return;
        ...
        <div className="flex-1 overflow-auto p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-sm font-semibold">
                {zh ? "分镜板" : "Storyboard"}{selectedSceneId ? "" : (zh ? " — 全部场景" : " — All Scenes")}
              </h2>
              <p className="text-xs text-studio-muted mt-0.5">
                {zh ? `${sortedShots.length} 个镜头 · 拖动排序` : `${sortedShots.length} shot${sortedShots.length !== 1 ? "s" : ""} · Drag to reorder`}
              </p>
            </div>
            <button
              onClick={() => setShowCreate(!showCreate)}
              disabled={!selectedSceneId}
              title={!selectedSceneId ? (zh ? "请先选择场景" : "Select a scene first") : (zh ? "新建镜头" : "Create new shot")}
          ...
          {showCreate && !selectedSceneId && (
            <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-xl text-xs text-yellow-400">
              {zh
                ? "请先从左侧选择场景，镜头需要继承场景中的角色、地点和定场画面"
          ...
          {sortedShots.length > 0 ? (
                ...
                >
                  ...
                  >
                    {shot.frame_image_path ? (
                      <Image unoptimized src={shot.frame_image_path} alt={shot.name} width={1280} height={720} className="w-full h-full object-contain" />
                    ) : (
                      <div className="flex flex-col items-center gap-1 text-studio-muted">
                        <Clapperboard className="w-6 h-6 opacity-40" />
                        <span className="text-[10px]">{zh ? "暂无画面" : "No frame"}</span>
                      </div>
                    )}
                    {shot.frame_image_path && (
                      <button
                        ...
                        title={zh ? "重新生成画面" : "Regenerate frame"}
                      >
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-sm text-studio-muted">{zh ? "还没有镜头" : "No shots yet"}</p>
              <p className="text-xs text-studio-muted/50 mt-1 mb-4">{zh ? "创建第一个镜头以开始分镜设计" : "Create your first shot to start the storyboard"}</p>
              <button
