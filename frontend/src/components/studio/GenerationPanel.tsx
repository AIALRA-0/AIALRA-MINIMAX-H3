...
export function GenerationPanel({ projectId }: { projectId: string }) {
  ...
  const handleGenerate = async () => {
    if (!prompt.trim()) {
      setError(t("asset.promptRequired"));
      return;
    }
    setGenerating(true);
    setError(null);
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <div className="mb-3">
        <h2 className="text-sm font-semibold">{t("asset.title")}</h2>
        <p className="text-[11px] text-studio-muted mt-0.5">{t("asset.subtitle")}</p>
      </div>
        ...
        <label className="block text-[10px] font-semibold text-studio-muted uppercase tracking-wider mb-1.5">
          ...
          <span className="normal-case opacity-50 ml-1.5">
            {currentDriver?.category === "cloud" ? (
              ...
              <span className="text-studio-success">{t("asset.local")}</span>
            ) : null}
          </span>
        </label>
        ...
        <div className="mb-3 p-2 bg-studio-panel rounded-lg border border-studio-border">
          <div className="flex items-center gap-2">
            {selectedAsset.primary_image && (
              <NextImage src={selectedAsset.primary_image} alt="" width={40} height={40} sizes="40px" unoptimized className="w-10 h-10 object-cover rounded-md shrink-0" />
            )}
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-semibold text-studio-muted uppercase tracking-wider">{t("asset.reference")}</p>
          ...
          {selectedAsset.generation_prompt && (
            <div className="mt-1.5 pt-1.5 border-t border-studio-border/50">
              <p className="text-[10px] font-semibold text-studio-muted uppercase tracking-wider mb-0.5">{t("asset.originalPrompt")}</p>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1.5">
          <label className="text-[10px] font-semibold text-studio-muted uppercase tracking-wider">{t("studio.prompt")}</label>
          <div className="flex items-center gap-2">
      ...
      <div className="flex gap-3 mb-3">
        ...
        <div>
          <label className="block text-[10px] font-semibold text-studio-muted uppercase tracking-wider mb-1.5">{t("asset.type")}</label>
          <select
            value={assetType}
        ...
        {showAdvanced && (
          <div className="mt-2 space-y-3 p-3 bg-studio-panel/50 rounded-lg border border-studio-border/50">
            ...
            <MegapixelsControl
              ...
              disabled={!currentDriver?.supports_megapixels}
            />
