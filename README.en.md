...
- H3 Continuum V3.8 long takes with native latent video/audio context, exact-duration assembly, chunk review, and resumable generation
...
- For a quality-first long take, use the 20-step base model and H3 Continuum with 22 frames of native continuation context
...
- Backend suite: `30 passed`
- Production frontend build, ESLint, and TypeScript checks passed
- Four ComfyUI API workflows passed validation against a live `object_info` response, including H3 Continuum V3.8 for long takes
...
- The previous three-independent-shot demo failed human continuity review and is retained only as regression evidence
- The current H3 Continuum candidate uses 3×5-second latent AV chunks at 1024×576 and 20 steps, producing 360 frames and 14.998 seconds in about 690.5 seconds
- The 5-second and 10-second boundaries pass frame-by-frame continuity review, but the candidate is not presented as an industry-best high-resolution final
- The current candidate retains 14.999 seconds of H.264 video and AAC stereo audio; sampled frames preserve the character, wardrobe, compass, glasshouse, lighting, and camera progression
