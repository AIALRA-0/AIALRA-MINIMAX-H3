...
- 连续长镜头使用 H3 Continuum V3.8，在生成阶段传递视频与音频 latent 上下文，输出阶段只做精确裁切和顺序装配
...
- 质量优先长镜头改用 H3 Continuum、20 步基础模型和 22 帧上下文，3×5 秒分块在同一条 latent AV 序列中续写
...
- 四份 ComfyUI API 工作流通过真实 `object_info` 节点和选项校验，其中长镜头使用 H3 Continuum V3.8
...
- 旧三段独立生成加 xfade 的样片已经撤销质量版结论，它只能证明编码链可运行，不能证明语义运动连续
- H3 Continuum 低分辨率功能门为 736×416、2×4 秒、20 步，耗时约 180 秒，原生边界通过逐帧检查
- H3 Continuum 正式候选为 1024×576、3×5 秒、20 步、360 帧、14.998 秒，耗时约 690.5 秒
- 5 秒与 10 秒边界没有构图重置、人物替换或硬切，人物、服装、指南针、温室结构与声场连续
- 正式候选通过连贯性门槛，但暗部、皮肤与细节仍属于 1024×576 母片水平，不宣称达到行业最佳终片
