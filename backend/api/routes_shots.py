...
# Persist the app-to-Comfy job mapping so browser polling survives restarts
_video_jobs = PersistentJobMap("shot_video_jobs")
...
async def check_video_status(job_id: str, model_id: str = "minimax_h3"):
    ...
    if response.status == GenerationStatus.COMPLETED and response.video_url:
      try:
        ...
        if job_info:
            ...
            # H3 produces 362 frames for a requested 15 seconds because valid
            # lengths follow 17k+5. Keep the raw master and expose an exact
            # 360-frame, 24 fps delivery copy to the browser and timeline.
            if req_data.get("model_id") == "minimax_h3" and req_data.get("duration_seconds") == 15.0:
                try:
                    raw_file = resolve_asset_url(local_path)
                    delivery_file = raw_file.with_name(f"take_{take_id}_15s.mp4")
                    normalize_delivery(raw_file, delivery_file, duration_seconds=15.0, fps=24)
                    local_path = f"/assets/{project_id}/shots/{shot_id}/{delivery_file.name}"
                except (ValueError, MediaPipelineError) as error:
                    print(f"[routes_shots] exact 15s delivery normalization failed: {error}")
