...
class ComfyVideoDriver(VideoDriver):
    ...
    def __init__(
        ...
        self._jobs = PersistentJobMap(f"comfy_video:{model_id}")
    ...
    def _resolve_local_path(self, path: str, return_full_path: bool = False) -> str:
        ...
        if not path.startswith("/assets/"):
            return path
        # Resolve through the shared containment check before touching the Vault.
    ...
    async def generate(self, request: VideoGenerationRequest) -> VideoGenerationResponse:
        ...
        try:
            async with self._get_session() as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        ...
                        job["error_message"] = f"ComfyUI error: {error_text}"
            ...
            job = self._jobs[job_id]
            job["status"] = GenerationStatus.FAILED
            job["error_message"] = f"Failed to connect to ComfyUI at {self.comfy_url}: {str(e)}"
            self._jobs[job_id] = job
            return VideoGenerationResponse(
                job_id=job_id,
                status=GenerationStatus.FAILED,
                            ...
                            if status.get("status_str") == "error":
                                ...
                                for message in reversed(messages):
                                    if isinstance(message, list) and len(message) > 1 and isinstance(message[1], dict):
                                        detail = message[1]
                                        error_message = detail.get("exception_message") or detail.get("error") or error_message
                                        if error_message != "ComfyUI execution failed":
                                            break
                                job["status"] = GenerationStatus.FAILED
